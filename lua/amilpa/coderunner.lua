...
api.nvim_create_autocmd("FileType", { pattern = "java", command = [[nnoremap <buffer><silent> <A-g> :TermExec cmd="javac %:t && java %:t:r" dir=%<CR>]] })
api.nvim_create_autocmd("FileType", { pattern = "javascriptreact", command = [[nnoremap <buffer><silent> <A-g> :TermExec cmd="npm run dev" dir=%<CR>]] })
